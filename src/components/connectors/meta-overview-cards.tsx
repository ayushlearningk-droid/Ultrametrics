...
/* ─── illustrative campaign rows (clearly marked Example) ────────── */

const EXAMPLE_CAMPAIGNS = [
  { id: 1, name: "Retargeting — Site Visitors", spend: 71148, ctr: 2.34, trendPct: 8.2 },
  { id: 2, name: "Summer Sale — Prospecting", spend: 94416, ctr: 1.91, trendPct: 12.4 },
  { id: 3, name: "Lookalike — 1% IN", spend: 182616, ctr: 1.68, trendPct: 5.6 },
  { id: 4, name: "Brand Awareness — Video", spend: 57960, ctr: 1.42, trendPct: -3.1 },
];
...
function healthInfo(score: number) {
  if (score >= 80) return { label: "Excellent", tone: "text-brand", bar: "bg-brand" };
  if (score >= 65) return { label: "Good", tone: "text-brand/80", bar: "bg-brand/70" };
  if (score >= 45) return { label: "Fair", tone: "text-warn", bar: "bg-warn" };
  return { label: "Poor", tone: "text-danger", bar: "bg-danger" };
}
...
export function MetaOverviewCards({
  ...
  return (
    <div className="space-y-6 anim-settle">
            ...
            <div key={m.label} className="panel panel-hover overflow-hidden p-5">
              <div className="flex items-start justify-between">
                ...
                {m.delta != null && (
                  <span
                    className={cn(
                      "rounded-full px-2 py-0.5 font-mono type-caption tabular-nums",
          ...
          <div className="min-w-0 flex-1">
            {loading ? (
              ...
              <>
                ...
                {/* Evidence chips — replace confidence/impact */}
      ...
      <div>
        <div className="mb-3 flex items-center gap-2.5">
          <span className="relative flex h-2 w-2 items-center justify-center">
            ...
            <span className="relative inline-flex h-[5px] w-[5px] rounded-full bg-brand" />
          </span>
          <p className="type-eyebrow text-foreground-muted/80">Campaign health</p>
          <span className="rounded-full border border-white/[0.1] bg-white/[0.03] px-2 py-0.5 type-caption text-foreground-muted">
            Example data
          </span>
        </div>
        <div className="panel overflow-hidden">
          {EXAMPLE_CAMPAIGNS.map((c, i) => {
            ...
            return (
              <div
                key={c.id}
                className={cn(
                  "flex items-center gap-5 px-5 py-4 transition-colors hover:bg-white/[0.02]",
                  i < EXAMPLE_CAMPAIGNS.length - 1 && "border-b border-white/[0.05]"
                )}
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate type-body font-medium text-foreground/88">
                    {c.name}
                  </p>
                  <div className="mt-2 flex items-center gap-2.5">
                    <div className="h-[3px] w-24 overflow-hidden rounded-full bg-white/[0.07]">
                      <div className={cn("h-full rounded-full", h.bar)} style={{ width: `${score}%` }} />
                    </div>
                    <span className={cn("type-caption font-medium", h.tone)}>{h.label}</span>
                  </div>
                </div>
                <div className="hidden w-20 text-right sm:block">
                  <p className="type-caption text-foreground-muted/60">CTR</p>
                  <p
                    className={cn(
                      "mt-0.5 font-mono type-body tabular-nums",
                      c.ctr >= 2 ? "text-brand" : c.ctr >= 1.5 ? "text-foreground/80" : "text-warn"
                    )}
                  >
                    {c.ctr.toFixed(2)}%
                  </p>
                ...
                <div className="hidden w-28 text-right sm:block">
                  <p className="type-caption text-foreground-muted/60">Spend</p>
                  <p className="mt-0.5 font-mono type-body tabular-nums text-foreground/75">
                ...
                <div className="flex w-24 items-center justify-end gap-1.5">
                  ...
                  <span
                    className={cn(
                      ...
                      c.trendPct > 0 ? "text-brand" : "text-danger"
                    )}
                  >
                    {c.trendPct > 0 ? "+" : ""}
      ...
      {/* ═══ ROW 5 · Google Sheets destination status ══════════════ */}
      <div>
        <div className="mb-3 flex items-center gap-2.5">
          <FileSpreadsheet className="h-3.5 w-3.5 text-foreground-muted/70" />
          <p className="type-eyebrow text-foreground-muted/80">Destination · Google Sheets</p>
        </div>
        <div className="panel grid grid-cols-2 gap-px overflow-hidden bg-white/[0.04] lg:grid-cols-4">
          {[
            ...
            {
              label: "Rows synced",
              value: totalImp != null ? formatNumber(recent.length || 0, currency) : "—",
            },
            {
              label: "Status",
              value: connector.status === "active" ? "Healthy" : "Paused",
                ...
                {"status" in cell && (
                  <span
                    className={cn(
                      ...
                      cell.status ? "bg-brand shadow-[0_0_6px_1px] shadow-brand/40" : "bg-warn"
                    )}
                  />
                )}
